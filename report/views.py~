...
def drinking_water(request):
    ...
    job =Job.objects.get(id=request.GET['id'])
    Drinking_waterFormSet = formset_factory(Drinking_waterForm, max_num=30, formset=RequiredFormSet)
	...
	if form1.is_valid():
        ...
        # Create a formset from the submitted data
        drink_water_formset = Drinking_waterFormSet(request.POST, request.FILES)
        
        if form1.is_valid() and drink_water_formset.is_valid():
	   report = form1.save(commit=False)
	   report.save()
           for form in drink_water_formset.forms:
                drink_water = form.save(commit=False)
                drink_water.Report_id = report
		drink_water.ip_address = request.META['REMOTE_ADDR']
                drink_water.save()
    ...
    # For CSRF protection
    c = {'report_form': report_form,
         'drink_water_formset': drink_water_formset,
        }
    c.update(csrf(request))
    
    return render_to_response('report/index.html', c)

def result_Drinking_water(request):
	Id = Drinking_water.objects.aggregate(Max('Report_id'))
	ID = Id['Report_id__max']
	st = Drinking_water.objects.filter(Report_id = ID)
	Id = Report.objects.aggregate(Max('id'))
	ID = Id['id__max']
	...
	organisation = Organisation.objects.all().filter(id = 1)
	department = Department.objects.all().filter(id = 1)
	return render_to_response('report/drink_water.html', {'st': st, 'Head':Head, 		'organisation':organisation,'department':department,'client':client,},context_instance=RequestContext(request))
 

def wall_tiles(request):
    # This class is used to make empty formset forms required
    class RequiredFormSet(BaseFormSet):
        def __init__(self, *args, **kwargs):
            super(RequiredFormSet, self).__init__(*args, **kwargs)
    ...
    job =Job.objects.get(id=request.GET['id'])
    Wall_TilesFormSet = formset_factory(Wall_TilesForm, max_num=30, formset=RequiredFormSet)
	...
	if form1.is_valid():
        ...
        # Create a formset from the submitted data
        walltiles_formset = Wall_TilesFormSet(request.POST, request.FILES)
        
        if form1.is_valid() and walltiles_formset.is_valid():
	   report = form1.save(commit=False)
	   report.save()
           for form in walltiles_formset.forms:
                walltiles = form.save(commit=False)
                walltiles.Report_id = report
		walltiles.ip_address = request.META['REMOTE_ADDR']
                walltiles.save()
	
           #return HttpResponseRedirect('thanks') # Redirect to a 'success' page
	   return HttpResponseRedirect(reverse('Automation.report.views.result_wfTiles'))
    else:
        report_form = ReportForm()
        walltiles_formset = Wall_TilesFormSet()
    
    # For CSRF protection
    c = {'report_form': report_form,
         'walltiles_formset': walltiles_formset,
        }
    c.update(csrf(request))

def floor_tiles(request):
    # This class is used to make empty formset forms required
    class RequiredFormSet(BaseFormSet):
        def __init__(self, *args, **kwargs):
            super(RequiredFormSet, self).__init__(*args, **kwargs)
    ...
    job =Job.objects.get(id=request.GET['id'])
    Floor_TilesFormSet = formset_factory(Floor_TilesForm, max_num=30, formset=RequiredFormSet)
	...
	if form1.is_valid():
        ...
        # Create a formset from the submitted data
        floortiles_formset = Floor_TilesFormSet(request.POST, request.FILES)
        
        if form1.is_valid() and flortiles_formset.is_valid():
	   report = form1.save(commit=False)
	   report.save()
           for form in floortiles_formset.forms:
                floortiles = form.save(commit=False)
                floortiles.Report_id = report
		floortiles.ip_address = request.META['REMOTE_ADDR']
                floortiles.save()
	   ...
	   return HttpResponseRedirect(reverse('Automation.report.views.result_wfTiles'))
        ...
        report_form = ReportForm()
        floortiles_formset = Floor_TilesFormSet()
    
    # For CSRF protection
    c = {'report_form': report_form,
         'floortiles_formset': floortiles_formset,
        }
    c.update(csrf(request))
    
    return render_to_response('report/index.html', c)

#def result_Wall_Tiles(request):
#	Id = Wall_Tiles.objects.aggregate(Max('Report_id'))
#	ID = Id['Report_id__max']
#	Id = Report.objects.aggregate(Max('id'))
#	ID = Id['id__max']
#	Head = Report.objects.filter(id = ID)
#	organisation = Organisation.objects.all().filter(id = 1)
#	return render_to_response('report/wftiles.html', {'dw': dw, 'Head':Head, 'organisation':organisation,},context_instance=RequestContext(request))

def result_wfTiles(request):
	Id = Floor_Tiles.objects.aggregate(Max('Report_id'))
	ID = Id['Report_id__max']
        Id = Wall_Tiles.objects.aggregate(Max('Report_id'))
	ID = Id['Report_id__max']
	Id = Report.objects.aggregate(Max('id'))
...
def coarse_aggregate(request):
    ...
    job =Job.objects.get(id=request.GET['id'])
    ca_FormSet = formset_factory(Coarse_AggregateForm, max_num=30, formset=RequiredFormSet)
    ...
    if request.method == 'POST': # If the form has been submitted...
        ...
        # Create a formset from the submitted data
        ca_formset = ca_FormSet(request.POST, request.FILES)

        if form1.is_valid() and ca_formset.is_valid():
           ca = form1.save(commit=False)
           ca.save()
           for form in ca_formset.forms:
                ca = form.save(commit=False)
                ca.Report_id = report
                ca.ip_address = request.META['REMOTE_ADDR']
                ca.save()

           #return HttpResponseRedirect('thanks') # Redirect to a 'success' page
           return HttpResponseRedirect(reverse('Automation.report.views.result_coarseaggregate'))
    else:
        report_form = ReportForm()
        ca_formset = ca_FormSet()
...
def result_coarseaggregate(request):
	Id = Coarse_Aggregate.objects.aggregate(Max('Report_id'))
	ID = Id['Report_id__max']
	ca = Coarse_Aggregate.objects.filter(Report_id = ID)
	Id = Report.objects.aggregate(Max('id'))
	ID = Id['id__max']
        
	Id = Report.objects.aggregate(Max('id'))
	ID = Id['id__max']
	job = Report.objects.filter(id = ID)
	client = Job.objects.all().filter(id=job.job_id).values('client__client__first_name',
	'client__client__middle_name', 'client__client__last_name',
	'client__client__address', 'client__client__city', 'date', 'letter_no', 
	'letter_date','clientjob__material__name',)
	...
	organisation = Organisation.objects.all().filter(id = 1)
	department = Department.objects.all().filter(id = 1)
	return render_to_response('report/coarse_aggregate.html', {'st': st, 'Head':Head, 		'organisation':organisation,'department':department,'client':client,},context_instance=RequestContext(request))
