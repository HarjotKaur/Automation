...
def result_Drinking_water(request):
	...
	ID = Id['Report_id__max']
	dw = Drinking_water.objects.filter(Report_id = ID)
	Id = Report.objects.aggregate(Max('id'))
	ID = Id['id__max']
	Head = Report.objects.filter(id = ID)
	organisation = Organisation.objects.all().filter(id = 1)
	return render_to_response('report/drink_water.html', {'dw': dw, 'Head':Head, 'organisation':organisation,},context_instance=RequestContext(request))
